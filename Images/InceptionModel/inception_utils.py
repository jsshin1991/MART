...
def load_model(model_path):
  ...
  graph_def = tf.GraphDef().FromString(tf.gfile.FastGFile(model_path,'rb').read())
...
def supplement_graph(graph):
  ...
  with graph.as_default():
    ...
    return inp, label_index, T(graph, 'softmax2')


def make_predictions(sess, graph):
  ...
  method.
  """
  inp, label_index, predictions = supplement_graph(graph)
  run_graph = sess.make_callable(predictions, feed_list=[inp, label_index])
